...
describe('useBookForm', () => {
  it('Автозаполняет форму при изменении поля name', async () => {
  ...
  it('По-дефолту делает соавтора пустым', async () => {
    const bookRepositoryStub = mock<BookRepository>();
    const notifyMock = mock<typeof notify>();
  ...
  it('Делает соавтора обязательным, если isPresentCoAuthor true', async () => {
    const bookRepositoryStub = mock<BookRepository>();
    const notifyMock = mock<typeof notify>();
