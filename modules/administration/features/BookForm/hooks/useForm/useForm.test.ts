...
describe('useBookForm', () => {
  it('При изменении имени книги происходит автозаполнения полей формы', async () => {
  ...
  it('По-дефолту соавтор отсутствует', async () => {
    const bookRepositoryStub = mock<BookRepository>();
    const notifyMock = mock<typeof notify>();
  ...
  it('Соавтор обязателен, если isPresentCoAuthor true', async () => {
    const bookRepositoryStub = mock<BookRepository>();
    const notifyMock = mock<typeof notify>();
